...
$(document).ready(function() {
  ...
  var id_sub_opd = `<?=$contentData['id_sub_opd']?>`;
  ...
  function getRealisasiOPD(){
    return $.ajax({
      url: "<?php echo site_url('SyncronizationController/getRealisasiSubOPD')?>",
      data : {tahun: tahun, 'id_sub_opd': id_sub_opd},
      type: 'GET',
      success: function (data){
        ...
        var realisasi_fisik = realisasi['realisasi_fisik'] != 0 ? `${realisasi['realisasi_fisik']}%` : 'n/a';
        $('#realisasi_fisik').html(realisasi_fisik);
      },
      error: function(e) {}
    });
  }
  ...
  function getAllRealisasiProgram(){
    return $.ajax({
      url: "<?php echo site_url('SyncronizationController/getAllRealisasiProgram')?>",
      data : {tahun: tahun, id_sub_opd: id_sub_opd},
      type: 'GET',
      success: function (data){
        var json = JSON.parse(data);
  ...
  function renderRealisasiProgram(data){
    ...
    Object.values(data).forEach((r) => {
      var detailBtn = `<a href="<?=site_url("PimpinanController/monevProgram/")?>?id_program_renja=${r['id_program_renja']}" class="btn btn-success btn-xs">Detail</a>`;
      renderData.push([`${r['id_program_renja']}::${r['nama_program_renja']}`, `${convertToRupiah(r['anggaran'])}`, `${convertToRupiah(r['realisasi_keuangan'])}`, coloriseRealisasi(r['capaian_keuangan']), 
