    ...
    <div class="modal-content animated fadeIn">
        ...
        <div class="modal-body" id="modal-body">              
          <form role="form" id="hasil_form" onsubmit="return false;" type="multipart">
            ...
            <button class="btn btn-success my-1 mr-sm-2" type="submit" id="save" data-loading-text="Loading..."><strong>Kirim ke Laboratorium</strong></button>
          </form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-white" data-dismiss="modal">Close</button>
...
$(document).ready(function() {
  var sessionData = JSON.parse(`<?=json_encode(DataStructure::slice($this->session->userdata(), ['id_role']))?>`);
   $('#terjadwal_sampel').addClass('active');
  ...
  function getAllSampel(){
    return $.ajax({
      ...
      data: {instansi : id_instansi, tahap: '2'},
...
function convStatus(status,hasil){
  ...
  function renderSampel(data){
    ...
    Object.values(data).forEach((sampel) => {
      ...
      var hasilButton = `<button class="hasil btn btn-success my-1 mr-sm-2"  data-id='${sampel['id_record']}' ><i class='fa fa-pencil'></i> No Sampel </button>`;
    ...
    //   var photo = sampel['photo_tim'] ? `<img src="<?=base_url('uploads/photo/')?>${sampel['photo_tim']}" class="img-sm">` : 'Tidak Ada';
      renderData.push([sampel['nama']+' / '+ sampel['NIK'],sampel['jenis'], sampel['no_sampel'],sampel['tanggal_pengambilan_sampel'],sampel['tahap'] == '2' ? detailButton+hasilButton : '']);
    });
