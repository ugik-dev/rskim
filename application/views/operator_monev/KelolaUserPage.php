  ...
  <div class="ibox ssection-container">
    <div class="ibox-content">
      <form class="form-inline" id="toolbar_form" onsubmit="return false;">
        ...
        <select class="form-control mr-sm-2" name="id_opd" id="id_opd" style="max-width:300px"></select>
        <button type="button" class="btn btn-success my-1 mr-sm-2" id="new_btn" disabled="disabled"><i class="fal fa-plus"></i> Tambah Tim Kegiatan</button>
      </form>
    </div>
  </div>
            ...
            <table id="FDataTable" class="table table-bordered table-hover" style="padding:0px">
              <thead>
                <tr>
                  ...
                  <th style="width: 16%; text-align:center!important">Foto</th>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content animated fadeIn">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Kelola Tim Kegiatan</h4>
      ...
      <div class="modal-body" id="modal-body">              
        <form opd="form" id="user_form" onsubmit="return false;" type="multipart" autocomplete="off">
          ...
          <input type="password" placeholder="Password" class="form-control" id="password" name="password" value="17agustus1945" style="display:none">
          <select class="form-control mr-sm-2" name="id_role" id="id_role" required="required" style="display:none"></select>
          <select class="form-control mr-sm-2" name="id_opd" id="opd" style="display:none"></select>
          <div class="form-group">
            <label for="no_hp">Nomor HP</label> 
            <input type="text" placeholder="Nomor HP" class="form-control" id="no_hp" name="no_hp">
          </div>
          <div id="photo"></div>
          <button class="btn btn-success my-1 mr-sm-2" type="submit" id="add_btn" data-loading-text="Loading..."><strong>Tambah Tim</strong></button>
...
$(document).ready(function() {
  ...
  $.when(getAllRole(), getAllOPD()).then((e) =>{
    getAllUser();
    toolbar.newBtn.prop('disabled', false);
    toolbar.id_role.attr('readonly', true);
    UserModal.id_role.attr('readonly', true);
  }).fail((e) => { console.log(e) });

  function getAllRole(){
    return $.ajax({
      url: `<?php echo site_url('SharedController/getAllRole/')?>`, 'type': 'POST',
      data: {id_role: 7},
  ...
  function getAllOPD(){
    return $.ajax({
      url: `<?php echo site_url('SharedController/getAllOPD/')?>`, 'type': 'GET',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderOPDSelectionFilter(data){
    ...
    toolbar.id_opd.append($('<option>', { value: "", text: "-- Semua User --"}));
    toolbar.id_opd.hide();
    Object.values(data).forEach((d) => {
  ...
  toolbar.id_opd.on('change', (e) => {
    UserModal.opd.attr('readonly', !empty(toolbar.id_opd.val()));
    getAllUser();
  });
  
  UserModal.id_role.on('change', (e) => {
    UserModal.opd.attr('required', ["4", "6"].includes(UserModal.id_role.val()));
  });
  ...
  function renderUser(data){
    ...
    Object.values(data).forEach((user) => {
      ...
      `;
      var no_hp = user['no_hp'] ? user['no_hp'] : 'Tidak Ada';
      var photo = user['photo'] ? `<img src="<?=base_url('uploads/photo/')?>${user['photo']}" class="img-sm">` : 'Tidak Ada';
      renderData.push([user['nip'], user['nama'], no_hp, photo, button]);
    });
  ...
  FDataTable.on('click', '.edit', function(){
    ...
    UserModal.photo.updateCurrentFile(currentData['photo']);
  });
