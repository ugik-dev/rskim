...
<button <?php if($this->session->userdata('id_role') == '3') echo 'hidden'?> style="width: 20%" class="btn btn-success my-3 mr-sm-3" type="button" id="edit_tahap" data-loading-text="Loading..."  ><strong>Evaluasi</strong></button>
 <form role="form" id="user_form" onsubmit="return false;" type="multipart" autocomplete="off">
<button <?php if($this->session->userdata('id_role') == '3') echo 'hidden'?> style="width: 20%" class="btn btn-success my-3 mr-sm-3" type="submit" id="save_edit_btn" data-loading-text="Loading..."  ><strong>Simpan</strong></button>
...
$(document).ready(function() {
  ...
  btnTahap.on('click', function(){
    tahapModal.self.modal('show');
    tahapModal.id_record.val(dataRecord['id_record'])
  });
...
// });
  ...
  tahapModal.form.on('submit', (ev) => {
     buttonLoading(btnUpdate);
    $.ajax({
          url: "<?=site_url('DinkesController/editRecord')?>", 'type': 'POST',
          data: tahapModal.form.serialize(),
          success: function (data){
            // buttonIdle(btnUpdate);
            var json = JSON.parse(data);
            if(json['error']){
...
function convertDateTime2(date){
  ...
  tahapModal.tahap.on('change', (e) => {
    if(tahapModal.tahap.val() != '2'){
      tahapModal.tanggal_pengambilan_sampel.prop('disabled',true)
      tahapModal.instansi.prop('disabled',true)
      tahapModal.instansi.val('')
      tahapModal.jenis_sampel.prop('disabled',true)
      tahapModal.jenis_sampel.val('')
    }else{
      tahapModal.tanggal_pengambilan_sampel.prop('disabled',false)
      tahapModal.instansi.prop('disabled',false)
      tahapModal.jenis_sampel.prop('disabled',false)
...
renderJenisTahap();
  function renderJenisTahap(data){
    tahapModal.tahap.empty();
    tahapModal.tahap.append($('<option>', { value: "", text: "Pilih Hasil Evaluasi"}));
    tahapModal.tahap.append($('<option>', { value: "2", text: "Rekomendasi Uji Sampel dan Laboratorium"}));
    tahapModal.tahap.append($('<option>', { value: "99", text: "Tidak Perlu Uji Sampel dan Laboratorium"}));
    tahapModal.jenis_sampel.empty();
    tahapModal.jenis_sampel.append($('<option>', { value: "", text: "Pilih Jenis Uji"}));
    tahapModal.jenis_sampel.append($('<option>', { value: "SWAP", text: "SWAP"}));
    tahapModal.jenis_sampel.append($('<option>', { value: "RAPID", text: "RAPID"}));
  }
    ...
    return $.ajax({
      ...
      success: function (data){
        ...
        getPasien(dataRecord['id_pasien'])
          ...
          if( dataRecord['tahap'] == '1' ){
            if(sessionData['id_role'] == 2){
              // info.edit_info_btn.hide();
              btnUpdate.hide();
              btnTahap.show();
            }else  if(sessionData['id_role'] == 6){
              btnUpdate.show();
              btnTahap.hide();
            }else{
              btnUpdate.hide();
      ...
      htmlinner +=`
            <div class="form-row">
              ...
              <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" value="${dataRecord['hasil_labor']}" readonly>
              </div>
            </div>`;
