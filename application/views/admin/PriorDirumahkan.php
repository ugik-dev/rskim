  ...
  <div class="ibox ssection-container">
    <div class="ibox-content">
      <form class="form-inline" id="toolbar_form" onsubmit="return false;">
        ...
        <input type="hidden" id="prior" name="prior" value="true">
      
        <select class="form-control mr-sm-2" name="kd_kab" id="kd_kab"></select>
        <select class="form-control mr-sm-2" name="kd_perusahaan" id="kd_perusahaan" style="max-width:300px"></select>
        <select class="form-control mr-sm-2" name="ada_pkh" id="ada_pkh" style="max-width:300px" disabled></select>
        <button hidden type="button" class="btn btn-success my-1 mr-sm-2" id="new_btn" disabled="disabled"><i class="fal fa-plus"></i> Tambah Data Dirumahkan</button>
      </form>
    </div>
  </div>
            ...
            <table id="FDataTable" class="table table-bordered table-hover" style="padding:0px">
              <thead>
                <tr>
                  ...
                  <th style="width: 24%; text-align:center!important">Status PKH</th>
...
$(document).ready(function() {
  $('#prior_dirumahkan').addClass('active');

  var toolbar = {
    'form': $('#toolbar_form'),
    'kd_kab': $('#toolbar_form').find('#kd_kab'),
    'nama_perusahaan': $('#toolbar_form').find('#nama_perusahaan'),
    'ada_pkh': $('#toolbar_form').find('#ada_pkh'),
    'kd_perusahaan': $('#toolbar_form').find('#kd_perusahaan'),
    'newBtn': $('#new_btn'),
    'prior': $('#toolbar_form').find('#prior'),
  }
  ...
  function renderRoleSelectionFilter(data){
    ...
    toolbar.kd_kab.append($('<option>', { value: "", text: "-- Pilih Kabupaten --"}));
  ...
  function getAllPerusahaan(){
    return $.ajax({
      url: `<?php echo site_url('TerdampakController/getPerusahaanDirumahkan/')?>`, 'type': 'POST',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
        ...
        renderOPDSelectionAdd(dataOPD);
      },
      error: function(e) {}
    });
  }
 
  function renderPKHFilter(data){
    toolbar.ada_pkh.empty();
    toolbar.ada_pkh.append($('<option>', { value: "", text: "-- Semua PKH --"}));
    toolbar.ada_pkh.append($('<option>', { value: "1", text: "Ya"}));
    toolbar.ada_pkh.append($('<option>', { value: "2", text: "Tidak"}));
  ...
  toolbar.kd_kab.on('change', (e) => {
    ...
    // toolbar.ada_pkh.prop('disabled', true);
    if(toolbar.kd_kab.val() == ""){
      toolbar.ada_pkh.val(null);
      toolbar.ada_pkh.prop('disabled', true);
      toolbar.kd_perusahaan.val(null);
      toolbar.kd_perusahaan.prop('disabled', true);
      FDataTable.clear().draw();
    }else{
    toolbar.kd_perusahaan.val(null);
    toolbar.ada_pkh.val(null).trigger('change');
    toolbar.ada_pkh.prop('disabled', false);
    toolbar.kd_perusahaan.val(null);
    toolbar.kd_perusahaan.prop('disabled', false);
    getAllPerusahaan();
    renderPKHFilter();
  }
  });

  toolbar.ada_pkh.on('change', (e) => {
    getAllData();
  });

  toolbar.kd_perusahaan.on('change', (e) => {
    getAllData();
  });
  ...
  function getAllPerusahaan(){
    ...
    return $.ajax({
      ...
      success: function (data){
        ...
        renderPerusahaanSelectionFilter(dataPerusahaan);
      },
      error: function(e) {}
    });
  }
  function renderPerusahaanSelectionFilter(data){
    toolbar.kd_perusahaan.empty();
    toolbar.kd_perusahaan.append($('<option>', { value: "", text: "-- Pilih Perusahaan --"}));
    Object.values(data).forEach((d) => {
      toolbar.kd_perusahaan.append($('<option>', {
        value: d['NAMA_PERUSAHAAN'],
  ...
  function getAllData(){
    ...
    swal.showLoading();
    return $.ajax({
      url: `<?php echo site_url('TerdampakController/getAllPriorDirumahkan/')?>`, 'type': 'GET',
      data: toolbar.form.serialize(),
      success: function (data){
        swal.close();
        var json = JSON.parse(data);
  ...
  function renderUser(data){
    ...
    Object.values(data).forEach((user) => {
      ...
      var detailButton = `<a href="<?=site_url("AdminController/DetailData/")?>?NO=${user['NO']}&categori=Dirumahkan" class="dropdown-item"><i class='fa fa-pencil'></i> Detail Data</a>`;
      ...
      `;
      renderData.push([user['nama_kab'], user['NAMA_PERUSAHAAN'], user['NAMA_PEKERJA'], user['NO_KTP_NIK_PEKERJA'], pkh , button]);
    });
