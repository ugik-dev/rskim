...
$(document).ready(function() {
  $('#terdampak_dirumahkan').addClass('active');
  ...
  function getAllOPD(){
    return $.ajax({
      url: `<?php echo site_url('SharedController/getAllOPD/')?>`, 'type': 'POST',
      data: {},
      success: function (data){
        var json = JSON.parse(data);
        if(json['error']){
  ...
  function renderOPDSelectionAdd(data){
    ...
    Object.values(data).forEach((d) => {
      UserModal.opd.append($('<option>', {
        value: d['kd_perusahaan'],
        text: d['kd_perusahaan'] + ' :: ' + d['nama_opd'],
      }));
    });
  }
  ...
  toolbar.kd_kab.on('change', (e) => {
    ...
    // getAllData();
    // toolbar.kd_perusahaan.val(null).trigger('change');
    getAllPerusahaan()
  });

  toolbar.kd_perusahaan.on('change', (e) => {
    // UserModal.opd.attr('readonly', !empty(toolbar.kd_perusahaan.val()));
    getAllData();
  });
  ...
  function getAllData(){
    ...
    swal.showLoading();
    return $.ajax({
      url: `<?php echo site_url('TerdampakController/getAllDirumahkan/')?>`, 'type': 'GET',
      data: toolbar.form.serialize(),
      success: function (data){
        swal.close();
        var json = JSON.parse(data);
  ...
  function resetUserModal(){
    ...
    // UserModal.opd.val(toolbar.kd_perusahaan.val() != -1 ? toolbar.kd_perusahaan.val() : "");
