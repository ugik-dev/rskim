...
$(document).ready(function() {
  ...
  var ProfileModal = {
    ...
    'photo': new FileUploader($('#profile_modal').find('#photox'), "", "photo", ".jpg"),
  }
  ...
  var user = JSON.parse(`<?=json_encode(DataStructure::slice($this->session->userdata(), ['id_user', 'username', 'nama', 'title_role', 'nama_opd', 'photo']))?>`);
  var profile = {
  ...
  function renderProfile(user){
    ...
    profile.photo.attr('src', !empty(user['photo']) ? `<?=base_url('uploads/photo/')?>${user['photo']}` : `<?=base_url('assets/img/profile_small.jpg')?>`);
  }
  ...
  profileButton.on('click', () => {
    ...
    ProfileModal.photo.resetState();
    ProfileModal.photo.updateCurrentFile(currentData['photo']);
  });
  ...
  ProfileModal.form.submit(function(event) {
    ...
    swal(swalSaveConfigure).then((result) => {
      ...
      $.ajax({
        ...
        data: new FormData(ProfileModal.form[0]),
