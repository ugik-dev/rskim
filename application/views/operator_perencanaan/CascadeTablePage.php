<span style="position: fixed;z-index:99999;right: 0px;bottom: 0px;background: #f1f1f1;padding: 4px 12px;margin: 4px;box-shadow: 1px 1px 2px 0px #000000cc;border-bottom: dashed;">
  <span class='rpjmd-text'>Sasaran RPJMD</span> | <span class='renstra-text'>Sasaran Renstra</span> | <span class='program-text'>Program PD</span> | <span class='kegiatan-text'>Kegiatan PD</span>
</span>
        ...
        <div class="ibox-content">
          <div class="table-responsive">
            <table id="FDataTable" class="table table-bordered" style="padding:0px">
              <thead>
                <tr>
                  <th style="width: 10%; text-align:center!important">Sasaran<br>RPJMD</th>
                  <th style="width: 10%; text-align:center!important">Sasaran<br>Indikator<br>RPJMD</th>
                  <th style="width: 10%; text-align:center!important">Sasaran<br>Renstra</th>
                  <th style="width: 10%; text-align:center!important">Sasaran<br>Indikator<br>Renstra</th>
                  <th style="width: 10%; text-align:center!important">Program PD</th>
                  <th style="width: 10%; text-align:center!important">Indikator<br>Program PD</th>
                  <th style="width: 13%; text-align:center!important">Kegiatan PD</th>
                  <th style="width: 17%; text-align:center!important">Indikator<br>Kegiatan PD</th>
                  <th style="text-align:center!important">Penganggung<br>jawab PD</th>
                </tr>
              </thead>
              <tbody></tbody>
            </table>
...
$(document).ready(function() {
  ...
  $('#cascade_result #table').addClass('active');
    ...
    Object.values(data['root']['sasaran_rpjmd']).forEach((rpjmd) => {
      ...
      Object.values(rpjmd['indikator_sasaran_rpjmd']).forEach((isr) => {
        ...
        Object.values(isr['sasaran_renstra']).forEach((sre) => {
          var belanja_sre = 'Belanja: ' + (sre['total_belanja_renstra'] ? `Rp${convertToRupiah(sre['total_belanja_renstra'])}` : 'Tidak ditemukan');
          var nama_sre = `<span class='renstra-text'>${sre['nama_sasaran_renstra']}</span> (${belanja_sre})`; 
          Object.values(sre['indikator_sasaran_renstra']).forEach((isre) => {
            var belanja_isre = 'Belanja: ' + (isre['total_belanja_indikator_renstra'] ? `Rp${convertToRupiah(isre['total_belanja_indikator_renstra'])}` : 'Tidak ditemukan');
            var nama_isre = `<span class='indikator-renstra-text'>${isre['nama_indikator_sasaran_renstra']} ::  ${isre['target_isre']}  ${isre['satuan_isre']}</span> (${belanja_isre})`; 
            Object.values(isre['program_renja']).forEach((pr) => {
              var belanja_pr = 'Belanja: ' + (pr['total_belanja_program'] ? `Rp${convertToRupiah(pr['total_belanja_program'])}` : 'Tidak ditemukan');
              var nama_pr = `<span class='program-text'>${pr['nama_program_renja']}</span> (${belanja_pr})`; 
              Object.values(pr['indikator_program_renja']).forEach((ipr) => {
                var belanja_ipr = 'Belanja: ' + (ipr['total_belanja_indikator_program'] ? `Rp${convertToRupiah(ipr['total_belanja_indikator_program'])}` : 'Tidak ditemukan');
                var nama_ipr = `<span class='indikator-program-text'>${ipr['nama_indikator_program_renja']} :: ${ipr['target_ipr']}  ${ipr['satuan_ipr']}</span> (${belanja_ipr})`; 
                Object.values(ipr['kegiatan_renja']).forEach((kr) => {
                  ...
                  Object.values(kr['indikator_kegiatan_renja']).forEach((ikr) => {
                    ...
                    renderData.push([nama_rpjmd, nama_isr, nama_sre, nama_isre, nama_pr, nama_ipr, nama_kr, nama_ikr, nama_opd]);
                  });
                  if(Object.keys(kr['indikator_kegiatan_renja']).length == 0) renderData.push([nama_rpjmd, nama_isr, nama_sre, nama_isre, nama_pr, nama_ipr, nama_kr, "-", "-"]);
                });
                if(Object.keys(ipr['kegiatan_renja']).length == 0) renderData.push([nama_rpjmd, nama_isr, nama_sre, nama_isre, nama_pr, nama_ipr, "-", "-", "-"]);
                
              });
              if(Object.keys(pr['indikator_program_renja']).length == 0) renderData.push([nama_rpjmd, nama_isr, nama_sre, nama_isre, nama_pr, "-", "-", "-", "-"]);

            });
            if(Object.keys(isre['program_renja']).length == 0) renderData.push([nama_rpjmd, nama_isr, nama_sre, nama_isre, "-", "-", "-", "-", "-"]);

          });
          if(Object.keys(sre['indikator_sasaran_renstra']).length == 0) renderData.push([nama_rpjmd, nama_isr, nama_sre, "-", "-", "-", "-", "-", "-"]);

        });
        if(Object.keys(isr['sasaran_renstra']).length == 0) renderData.push([nama_rpjmd, nama_isr, "-", "-", "-", "-", "-", "-", "-"]);
