    ...
    <div class="modal-content animated fadeIn">
      ...
      <form id="user_form" role="form" onsubmit="return false;" type="multipart" autocomplete="off">
          ...
          <div class="form-group">
            ...
            <select readonly class="form-control mr-sm-2" id="sl_puskesmas" name="id_puskesmas" required="required">
            </select>
          </div>
          <div class="form-group">
...
$(document).ready(function() {
  ...
  var id_puskesmas = `<?=$this->session->userdata('id_puskesmas')?>`;
  // var KDKAB = `<?=$this->session->userdata('KDKAB')?>`;
  ...
  toolbar.form.submit(function(event){
    ...
    switch(toolbar.form[0].target){
      ...
      case 'add':
        ...
        showPasienModal();
        PasienModal.sl_puskesmas.val(id_puskesmas);
  ...
  function getPasien(){
    ...
    var renderData = [];
    var url_detail = `<?php if($this->session->userdata('id_role') == '7'){
          echo 'RSController';
        }else{
          echo 'DinkesController';
        }  ?>`;
        console.log(url_detail)
    Object.values(data).forEach((pasien) => {
           var detailButton =`
      <a class="detail dropdown-item" href='<?=site_url()?>${url_detail}/DetailPasien?id_pasien=${pasien['id_pasien']}'><i class='fa fa-share'></i> Detail Pasien</a>
      `; 
      var editButton = `
