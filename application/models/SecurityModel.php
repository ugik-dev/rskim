...
class SecurityModel extends CI_Model {
  ...
  public function guestOnlyGuard($ajax = false){
    if($this->session->userdata('id_user')){
      if($ajax) throw new UserException('User tidak berhak mengakses resource ini', IS_AUTHENTICATED_CODE);
      redirect($this->session->userdata('nama_controller'));
    }
  }
  ...
  public function userOnlyGuard($ajax = false){
    if(!$this->session->has_userdata('id_user')){
      if($ajax) throw new UserException('Guest tidak berhak mengkases resource ini', IS_NOT_AUTHENTICATED_CODE);
      redirect('login');
    }
  }

  public function roleOnlyGuard($role, $ajax = false){
    $this->userOnlyGuard($ajax);
